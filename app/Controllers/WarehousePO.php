...
class WarehousePO extends BaseController
{
    ...
    public function verifyPoUnit()
    {
        if ($this->request->isAJAX()) {
            ...
            // Hapus nilai null agar tidak menimpa data yang sudah ada dengan kosong
                ...
                if ($this->pounitsmodel->update($id_unit, $dataToUpdate)) {
                    ...
                    if ($status === 'Sesuai') {
                        ...
                        if ($verifiedUnit) {
                            ...
                            // Map fields from PO Unit to inventory_unit schema (allowedFields in InventoryUnitModel)
                            $inventoryData = [
                                ...
                                'status_unit_id'     => 8, // 8 = STOCK NON-ASET
                                'lokasi_unit'        => 'POS 1',
                                ...
                                'model_attachment_id'=> $verifiedUnit['attachment_id'] ?? null,
                                'sn_attachment'      => $verifiedUnit['sn_attachment_po'] ?? null,
                                'model_baterai_id'   => $verifiedUnit['baterai_id'] ?? null,
                                'sn_baterai'         => $snData['sn_baterai_po'] ?: ($verifiedUnit['sn_baterai_po'] ?? null),
                                'model_charger_id'   => $verifiedUnit['charger_id'] ?? null,
                                'sn_charger'         => $verifiedUnit['sn_charger_po'] ?? null,
                            ...
                            if (!$this->inventoryUnitModel->insert($inventoryData)) {
                                ...
                                // PENTING: Hapus data dari po_units setelah berhasil dipindahkan ke inventory
                                $this->pounitsmodel->delete($id_unit);
                                log_message('info', '[WarehousePO] Successfully deleted unit ' . $id_unit . ' from po_units after transfer to inventory');
                            }
                        }
                    }
                    ...
                    $db->transRollback();
                    return $this->response->setJSON(['statusCode' => 500, 'message' => 'Gagal update database.']);
                }
            } catch (\Exception $e) {
