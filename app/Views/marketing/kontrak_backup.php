...
<style>
    ...
    #deleteConfirmationModal #deleteConfirmationText {
        border: 2px solid #dee2e6;
        transition: border-color 0.3s ease;
    }
    
    #deleteConfirmationModal #deleteConfirmationText:focus {
        border-color: #dc3545;
        box-shadow: 0 0 0 0.2rem rgba(220, 53, 69, 0.25);
    }
    
    #deleteConfirmationModal #confirmDeleteBtn:disabled {
        opacity: 0.5;
        cursor: not-allowed;
    }
...
// Global variables for custom pagination
...
$(document).ready(function() {
    ...
    // Disable SweetAlert monkey patch for this page to avoid duplicate notifications
    window._swalToastPatched = true;
    
    // Initialize the page
    // IMMEDIATE TEST: Check if basic DOM manipulation works
    try {
        const tbody = $('#contractsTable tbody');
        if (tbody.length > 0) {
            tbody.html('<tr><td colspan="7" class="text-center">Testing DOM manipulation...</td></tr>');
            // Clear and test with AJAX data
            setTimeout(function() {
                loadKontrakData();
            }, 500);
        } else {
    ...
    $('#kontrakSearch, #searchKontrak').on('input', function() {
...
function loadKontrakData() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.data) {
                // Convert DataTable format to simple array format
                allKontrakData = response.data.map(item => ({
                    id: item.id,
                    no_kontrak: item.contract_number?.replace(/<[^>]*>/g, '') || '', // Remove HTML tags
                    no_po_marketing: item.po || '',
                    pelanggan: item.client_name || '',
                    status: item.status?.replace(/<[^>]*>/g, '') || '', // Remove HTML tags
                    tanggal_mulai: item.period?.split(' - ')[0] || '',
                    tanggal_selesai: item.period?.split(' - ')[1] || '',
                    total_units: 0 // Will be updated from backend if needed
                }));
                
                // Update statistics from response stats if available
                if (response.stats) {
...
// Update statistics from DataTable response
function updateStatsFromResponse(stats) {
    $('#stat-total').text(formatNumber(stats.total || 0));
    $('#stat-active').text(formatNumber(stats.active || 0));
    $('#stat-expired').text(formatNumber(stats.expired || 0));
    $('#stat-expiring').text(formatNumber(stats.expiring || 0));
}
...
function deleteContract(contractId) {
    ...
    // Get contract data for confirmation
    $.ajax({
        ...
        success: function(response) {
            if (response.success && response.data) {
                const contract = response.data;
                showDeleteConfirmation(contractId, contract);
            } else {
                safeShowNotification('Gagal memuat data kontrak.', 'error');
            }
        },
...
function showDeleteConfirmation(contractId, contract) {
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-danger">
                    ...
                    <div class="modal-body">
                        ...
                        <h6 class="mb-3">Data Kontrak yang akan dihapus:</h6>
                        <div class="card bg-light">
                            <div class="card-body">
                                <table class="table table-sm table-borderless mb-0">
                                    <tr>
                                        <td class="fw-bold">No. Kontrak:</td>
                                        <td>${contract.no_kontrak || '-'}</td>
                                    </tr>
                                    <tr>
                                        <td class="fw-bold">Pelanggan:</td>
                                        <td>${contract.pelanggan || '-'}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>Rp ${new Intl.NumberFormat('id-ID').format(contract.nilai_total || 0)}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 p-3 bg-danger bg-opacity-10 border border-danger rounded">
                            ...
                            <ul class="text-danger mb-0 small">
                                <li>Kontrak akan dihapus permanen dari sistem</li>
                                <li>Data spesifikasi terkait akan ikut terhapus</li>
                                <li>Riwayat transaksi terkait akan terpengaruh</li>
                        ...
                        <div class="mt-4">
                            <div class="form-check">
                                ...
                                <label class="form-check-label fw-bold text-danger" for="confirmUnderstood">
                                    Saya memahami konsekuensi dan tetap ingin menghapus kontrak ini
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="mt-3">
                            <label class="form-label fw-bold">
                                Ketik "<span class="text-danger">HAPUS KONTRAK</span>" untuk konfirmasi:
                            </label>
                            <input type="text" class="form-control" id="deleteConfirmationText" 
                                   placeholder="Ketik: HAPUS KONTRAK" autocomplete="off">
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-danger" id="confirmDeleteBtn" disabled 
                                onclick="executeDelete(${contractId})">
                            <i class="fas fa-trash me-1"></i>Ya, Hapus Kontrak
                        </button>
                    </div>
                </div>
            </div>
    ...
    // Enable delete button only when both conditions are met
    function checkDeleteConditions() {
        const understood = $('#confirmUnderstood').is(':checked');
        const textMatch = $('#deleteConfirmationText').val().trim().toUpperCase() === 'HAPUS KONTRAK';
        $('#confirmDeleteBtn').prop('disabled', !(understood && textMatch));
    }
    
    $('#confirmUnderstood, #deleteConfirmationText').on('change keyup', checkDeleteConditions);
...
function executeDelete(contractId) {
...
function performDelete(contractId) {
    ...
    safeShowNotification('Menghapus kontrak...', 'info');
    
    // Disable delete button temporarily
    $('#confirmDeleteBtn').prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i>Menghapus...');
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $('#confirmDeleteBtn').prop('disabled', false).html('<i class="fas fa-trash me-1"></i>Ya, Hapus Kontrak');
            }
        },
        error: function(xhr, status, error) {
            );
            let errorMessage = 'Terjadi kesalahan saat menghapus kontrak.';
            
            if (xhr.status === 404) {
                errorMessage = 'Endpoint delete tidak ditemukan (404).';
            } else if (xhr.status === 500) {
                errorMessage = 'Error server internal (500). Cek log untuk detail.';
            } else if (xhr.status === 0) {
                errorMessage = 'Koneksi terputus. Periksa koneksi internet.';
            } else if (status === 'timeout') {
                errorMessage = 'Request timeout. Coba lagi setelah beberapa saat.';
            }
