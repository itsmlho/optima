...
function initializeFormSubmission() {
    $('#createUserForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    createOptimaToast({ type: 'success', title: 'Berhasil', message: response.message || 'User berhasil dibuat', duration: 3000 });
                    // Redirect to edit page for the new user
                    setTimeout(() => {
                        if (response.user_id) {
                            window.location.href = BASE_URL + 'admin/advanced-users/edit/' + response.user_id;
                        } else {
                            window.location.href = USERS_LIST_URL;
                        }
                    }, 1500);
                } else {
                    createOptimaToast({ type: 'error', title: 'Gagal', message: response.message || 'Gagal membuat user', duration: 4000 });
