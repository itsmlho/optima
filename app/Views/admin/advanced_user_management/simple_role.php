        ...
        <div class="col-12">
            ...
            <div class="permission-info">
                ...
                <p>
                    ...
                    <strong>Edit:</strong> Can view and edit data (read + write) | 
                    <strong>Full:</strong> Can view, edit, and delete data (read + write + delete)
                </p>
            </div>
...
// Load all roles
function loadRoles() {
    fetch('<?= base_url('admin/simple-roles/get-roles') ?>')
...
function displayRoles(roles) {
    ...
    if (roles.length === 0) {
        rolesList.innerHTML = '<div class="text-center py-5"><p class="text-muted">No roles found</p></div>';
        return;
    }
        ...
        roleCard.innerHTML = `
            <div class="role-header">
                ...
                <div>
                    ...
                    <button class="btn btn-sm btn-outline-primary ms-2 btn-simple" onclick="editRole(${role.id})">
...
function editRole(roleId) {
    fetch(`<?= base_url('admin/simple-roles/get-role') ?>/${roleId}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function loadPermissions() {
    fetch('<?= base_url('admin/simple-roles/get-permissions') ?>')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function populateRoleForm(role, permissions) {
    ...
    // Load permissions and set checked states
    loadPermissions();
    
    // Set checked states based on role permissions after a short delay
    setTimeout(() => {
...
function saveRole() {
    ...
    fetch('<?= base_url('admin/simple-roles/save-role') ?>', {
        method: 'POST',
        headers: {
